...
static void printephdiff(gtime_t time, int sat, int eph1, int eph2,
                         const nav_t *nav, int topt, int eopt, int mopt,
                         const double *pos)
{
    ...
    if (!satpos(time,time,sat,eph1,nav,rs1,dts1,&var1,&svh1)) return;
    if (!satpos(time,time,sat,eph2,nav,rs2,dts2,&var2,&svh2)) return;
    if (svh1||svh2) return;
...
int main(int argc, char **argv)
{
    ...
    for (i=0;i<n;i++) {
       ...
       else if (!strcmp(ext,".eph")||!strcmp(ext,".EPH")) {
           readsp3(files[i],&nav);
       }
    ...
    for (i=0;i<(int)(tspan*3600.0/tint);i++) {
       ...
       for (s=1;s<=MAXSAT;s++) {
           ...
           printephdiff(time,s,eph1,eph2,&nav,topt,eopt,mopt,pos);
       }
    }
    if (fp) fclose(fp);
    if (trl>0) traceclose();
